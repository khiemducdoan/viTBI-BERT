...
class ViTBERT(Dataset):
    def __init__(self, data_path,stop_words_file, wordnet_file,indices, type , tokenizer=None):
        ...
        if type == "train":
            self.dataset = self.data_augmentor.augment_dataframe(self.dataset, num_aug=4, alpha=0.1, max_aug_for_3=80)
        else:
    ...
    def __getitem__(self, idx):
        text1 = self.dataset.iloc[idx, 0]  # Extract text from dataframe
        text2 = self.dataset.iloc[idx, 1]  # Extract text from dataframe
        label = self.dataset.iloc[idx, 2]-1.0  # Extract label

        # Tokenization
        tokenized_text1 = self.tokenizer(text1, add_special_tokens=True, max_length=100, padding='max_length', truncation=True, return_tensors='pt')
        tokenized_text2 = self.tokenizer(text2, add_special_tokens=True, max_length=100, padding='max_length', truncation=True, return_tensors='pt')

        # Extract fields from tokenized text
        input_ids1 = tokenized_text1['input_ids'].squeeze()
        attention_mask1 = tokenized_text1['attention_mask'].squeeze()
        input_ids2 = tokenized_text2['input_ids'].squeeze()
        attention_mask2 = tokenized_text2['attention_mask'].squeeze()

        # Convert label to tensor
        # label = torch.tensor(label, dtype=torch.long)  # Changed dtype to torch.long

        # Return tensors
        return input_ids1, attention_mask1,input_ids2, attention_mask2, label, text1,text2  # Return attention_mask
